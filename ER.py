...
run = paragraph.add_run("日期:"+datetime.now().strftime('%Y-%m-%d'))
run.font.size = Pt(6)
...
driver.get("https://web9.vghtpe.gov.tw/ERS/queryRecord/part/"+ER_depart)
soup = BeautifulSoup(driver.page_source, 'html.parser')
ER_table=soup.find("table")
ER_table=html_table(ER_table)
driver.get("https://web9.vghtpe.gov.tw/emr/qemr/qemr.cfm?action=findEmr&histno=50687768")
soup = BeautifulSoup(driver.page_source, 'html.parser')
ER_table=ER_table[["床號", "病歷號", "姓名", "年齡", "性別", "診斷描述", "科別","處置"]]
for i in range(len(ER_table)):
    pat=list(ER_table.iloc[i])
...
# 保存 Word 文件
filename=datetime.now().strftime('%Y%m%d')+"_"+"patient_list"+'.docx'
doc.save(filename)
print("儲存為"+filename)
